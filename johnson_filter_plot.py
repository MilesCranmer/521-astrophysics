...
plt.figure(1)
ax = plt.subplot(211)
ax.scatter(nm_wavelengths, y_positions, color='black', marker='o')
ax.set_xlabel(r"$\lambda_c=$ Central effective wavelength $ {\rm(nm)}$")
ax.set_xscale('log')
ax.set_xlim(min(nm_wavelengths)/1.5, max(nm_wavelengths)*1.5)
ax.set_yticks(y_positions, [r"$\rm"+name+" $" for name in filter_names])
#plt.ticklabel_format(axis='both')
ax.set_ylabel("Johnson filter")

ax = plt.subplot(212)
ax.scatter(nm_wavelengths, Y, color='black', marker='o')
ax.set_xlabel(r"$\lambda_c=$ Central effective wavelength $ {\rm(nm)}$")
ax.set_xscale('log')
ax.set_xlim(min(nm_wavelengths)/1.5, max(nm_wavelengths)*1.5)
ax.set_ylabel(r"$F_\lambda=$ Flux density  $(\mathrm{ W m^{-2} \mu m^{-1}})$ ")
ax.set_yscale('log')
